...
import { NTPMeasurement, calculateOffsetEstimate } from './ntp';
 ...
 */
export function useWebSocketWithNTP(url: string) {
  ...
  const handleNTPResponse = useCallback((data: { t0: number; t1: number; t2: number }) => {
    ...
    ntpMeasurementsRef.current = [...ntpMeasurementsRef.current, measurement].slice(
